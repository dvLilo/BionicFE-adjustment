...
import { useLazyGetActivitiesQuery } from "../../features/activities/activities.slice"
...
const Activities = () => {
  ...
  const [getActivity, {
    ...
    originalArgs: activityArgs,

    isSuccess: isGetActivitySuccess,
    isUninitialized: isGetActivityUninitialized,
    isLoading: isGetActivityLoading,
    isFetching: isGetActivityFetching,
    isError: isGetActivityError
  }] = useLazyGetActivitiesQuery()

  const [exportActivity, {
    isLoading: isExportActivityLoading,
    isFetching: isExportActivityFetching,
  }] = useLazyGetActivitiesQuery()

  const [activity, setActivity] = useState({})

  const viewChangesHandler = (data) => {
    setActivity(data)
    onOpen()
  }
  ...
  const onExportHandler = async () => {
    ...
    toast({
      title: "Generating..",
      text: "This may take a few minutes. Feel free to keep working whilst we generate your reports.",
      icon: "info"
    })

    try {
      const res = await exportActivity(params).unwrap()

      if (activityArgs.type === "SUMMARIZED") {
        const data = res.map((item) => ({
          id: item.properties.old.ID,
          series_no: item.properties.old.series,
          category: item.properties.old.category.toUpperCase(),
          farm: item.properties.old.farm.toUpperCase(),
          building: item.properties.old.building.toUpperCase(),
          leadman: item.properties.old.leadman.toUpperCase(),
          checker: item.properties.old.checker.toUpperCase(),
          buyer: item.properties.old.buyer.toUpperCase(),
          plate_no: item.properties.old.plate_no.toUpperCase(),
          date_harvest: item.properties.old.current_date_in,
          total_heads: item.properties.old.total_heads,
          total_weight: item.properties.old.total_weight?.toFixed(2),
          date_activity: moment(item.created_at).format("YYYY-MM-DD HH:mm:ss"),
          causer: item.causer.full_name.toUpperCase()
        }))

        exportInformationHandler(data)
      }

      if (activityArgs.type === "DETAILED") {
        const data = res.map((item) => ({
          id: item.properties.old.id,
          transaction_no: item.properties.old.transaction_no,
          series_no: item.properties.old.series_no,
          category: item.properties.old.category.toUpperCase(),
          date_harvest: item.properties.old.current_date_in,
          farm: item.properties.old.farm.toUpperCase(),
          building: item.properties.old.building.toUpperCase(),
          ...
          buyer: item.properties.old.buyer.toUpperCase(),
          plate_no: item.properties.old.plate_no.toUpperCase(),
          heads: item.properties.old.heads,
          weight: parseFloat(item.properties.old.weight),
          allowance: item.properties.old.allowance?.toFixed(2),
          date_activity: moment(item.created_at).format("YYYY-MM-DD"),
          causer: item.causer.full_name.toUpperCase()
        }))
  ...
  const exportTransactionHandler = async (data) => {
    try {
      const workbook = new Workbook()
      const worksheet = workbook.addWorksheet("Bionic Distro Activity Report")

      const logo = workbook.addImage({
        base64: logoBase64,
        extension: "png"
      })
      worksheet.mergeCells("A1:P4")
      ...
      worksheet.columns = [
        ...
        { key: "heads", width: 12 },
        { key: "weight", width: 12 },
        { key: "allowance", width: 12 },
        { key: "causer", width: 16 },
        { key: "date_activity", width: 13 }
      ]
      ...
      await workbook.xlsx.writeBuffer().then((data) => {
        ...
        fs.saveAs(blob, "Bionic Distro Detailed Activity Report.xlsx");
      });
    } catch (error) {
      console.log(error)
      toast({
  ...
  const exportInformationHandler = async (data) => {
    try {
      const workbook = new Workbook()
      const worksheet = workbook.addWorksheet("Bionic Distro Acitivity Report")
      ...
      await workbook.xlsx.writeBuffer().then((data) => {
        ...
        fs.saveAs(blob, "Bionic Distro Summary Activity Report.xlsx");
      });
    } catch (error) {
      console.log(error)
      toast({
  ...
  return (
    <Box className="bioncTransaction">
      <Stack direction="row" justifyContent="space-between" alignItems="center">
        <Typography className="bioncTransaction__heading" variant="h5">Activity View</Typography>

        <FilterActivities errors={activitiesError?.data?.errors} onFilter={getActivity} />
      </Stack>
        ...
        <Table className="bioncTransaction__table">
          <TableHead className="bioncTransaction__thead">
            <TableRow className="bioncTransaction__row">
              ...
              <TableCell className="bioncTransaction__cell">TYPE</TableCell>
              <TableCell className="bioncTransaction__cell">EVENT</TableCell>
              <TableCell className="bioncTransaction__cell">CAUSER</TableCell>
              <TableCell className="bioncTransaction__cell">DESCRIPTION</TableCell>
              <TableCell className="bioncTransaction__cell">DATE</TableCell>
              <TableCell className="bioncTransaction__cell" align="center">ACTIONS</TableCell>
            </TableRow>
          </TableHead>

          {
            (isGetActivitySuccess && !isGetActivityLoading && !isGetActivityFetching) && (
              <TableBody className="bioncTransaction__tbody">
                {
                  activities?.data?.map((item, index) => (
                    <TableRow className="bioncTransaction__row" key={index} hover>
                      ...
                      <TableCell className="bioncTransaction__cell">
                        {item.causer.full_name?.toUpperCase()}
                      </TableCell>
                      ...
                      <TableCell className="bioncTransaction__cell">
                        {moment(item.created_at).format("MMMM DD, YYYY, hh:mm A")?.toUpperCase()}
                      </TableCell>

                      <TableCell className="bioncTransaction__cell" align="center">
                        <MenuActivities onView={() => viewChangesHandler(item)} />
                      </TableCell>
                    </TableRow>
                  ))
                }
          ...
          {
            (isGetActivityLoading || isGetActivityFetching) && (
              <TableBody className="bioncTransaction__tbody">
                <TableRow className="bioncTransaction__row">
          ...
          {
            (isGetActivityUninitialized || isGetActivityError) && (
              <TableBody className="bioncTransaction__tbody">
                <TableRow className="bioncTransaction__row">
                  <TableCell className="bioncTransaction__cell bioncTransaction__cell--empty" align="center" colSpan={15}>
                  ...
                  <TableCell className="bioncTransaction__cell" colSpan={15}>
                    <Stack direction="row" justifyContent="space-between" alignItems="center">
                      <LoadingButton
                        ...
                        loading={isExportActivityLoading || isExportActivityFetching}
                      ...
                      <TablePagination
                        ...
                        rowsPerPage={activities?.per_page}
