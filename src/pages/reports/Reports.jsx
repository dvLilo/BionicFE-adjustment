...
import { useLazyGetTransactionsQuery } from "../../features/transactions/transactions.slice"
...
const Reports = () => {
  ...
  const [exportTransaction, {
    isLoading: isExportTransactionLoading,
    isFetching: isExportTransactionFetching,
  }] = useLazyGetTransactionsQuery()

  const submitFilterHandler = async (data) => {
    const params = {
      page: "none",
    ...
    try {
      if (data.type === "SUMMARIZED") {
        const res = await exportInformation(params).unwrap()

        exportInformationHandler(res)
      }

      if (data.type === "DETAILED") {
        const res = await exportTransaction(params).unwrap()

        exportTransactionHandler(res)
      }
    } catch (error) {
      if (error.status === 422) {
  ...
  const exportTransactionHandler = async (data) => {
    try {
      ...
      worksheet.addRows(data.map((item) => ({
        ...
        leadman: item.leadman.toUpperCase(),
        checker: item.checker.toUpperCase(),
        buyer: item.buyer.toUpperCase(),
        plate_no: item.plate_no.toUpperCase(),
      ...
      await workbook.xlsx.writeBuffer().then((data) => {
        ...
        fs.saveAs(blob, "Bionic Distro Detailed Report.xlsx");
      });
    } catch (error) {
      console.log(error)
      toast({
  ...
  return (
    <Box className="bioncTransaction">
      <Stack direction="row" justifyContent="space-between" alignItems="center">
        <Typography className="bioncTransaction__heading" variant="h5">Generate Reports</Typography>
      </Stack>

      <Stack component="form" direction="row" alignItems="flex-start" gap={2} marginTop={4} onSubmit={handleSubmit(submitFilterHandler)}>
        ...
        <LoadingButton
          type="submit"
          variant="contained"
          loading={isExportTransactionLoading || isExportTransactionFetching || isExportInformationLoading || isExportInformationFetching}
...
const schema = yup.object().shape({
  type: yup.string().required().label("Type of report"),
